...
if ($IncludeAll -or !$included) {
    #Filte5p-][rable, but if we are going to include all the methods, we need to cycle all mailboxes anyway
    ...
    else { $MBList = Get-Mailbox -ResultSize Unlimited -RecipientTypeDetails UserMailbox,SharedMailbox,RoomMailbox,EquipmentMailbox,DiscoveryMailbox,TeamMailbox | Select-Object -Property Displayname,Identity,PrimarySMTPAddress,RecipientTypeDetails,ForwardingAddress,ForwardingSMTPAddress,DeliverToMailboxAndForward }
}
    ...
    else { $MBList = Get-Mailbox -ResultSize Unlimited -RecipientTypeDetails $Using:included | Select-Object -Property Displayname,Identity,PrimarySMTPAddress,RecipientTypeDetails,ForwardingAddress,ForwardingSMTPAddress,DeliverToMailboxAndForward }
}
